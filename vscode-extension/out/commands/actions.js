...
function createIngestWorkspaceHandler(context) {
    return async (resource) => {
        const workspaceRoot = resolveWorkspaceRoot(context.getWorkspaceFolder, resource ?? context.getActiveEditor()?.document.uri);
        if (!workspaceRoot) {
...
function createReindexWorkspaceHandler(context) {
    return async () => {
        const workspaceRoot = resolveWorkspaceRoot(context.getWorkspaceFolder, context.getActiveEditor()?.document.uri);
        if (!workspaceRoot) {
