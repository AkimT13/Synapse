...
exports.ReviewWebviewPanel = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("node:path"));
const VIEW_TYPE = "synapse.reviewPanel";
class ReviewWebviewPanel {
    panel;
    store;
    unsubscribe;
    constructor(store) {
    ...
    reveal() {
        ...
        this.panel = vscode.window.createWebviewPanel(VIEW_TYPE, "Synapse Review", { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, { enableScripts: true, retainContextWhenHidden: true });
        this.panel.webview.html = this.buildHtml(state.review, state.drift);
...
exports.ReviewWebviewPanel = ReviewWebviewPanel;
// --- HTML renderers ---
...
function renderHeader(target, status, workspace) {
    return `
...
function renderDriftEntry(entry) {
    const lineLink = entry.line_range
        ...
        : "";
    const findingsHtml = entry.findings.length > 0
        ...
        : "";
    const violationsHtml = entry.violations.length > 0
        ? `<div class="subsection">
...
function renderContextEntry(ctx) {
    const flags = [];
    if (ctx.has_conflict)
        flags.push("conflict");
...
function statusClass(status) {
    ...
    if (n.includes("align") || n.includes("ok") || n.includes("pass"))
        return "status-ok";
    if (n.includes("warn") || n.includes("review"))
        return "status-warn";
    return "status-error";
}
